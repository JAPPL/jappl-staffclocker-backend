...
class TestProjectMemberView(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        employee_2: UserDetail = user_instance.make()
        project_1: Project = project_instance.make()
        project_2: Project = project_instance.make()
        cls.employee_1_token: str = "Bearer " + str(RefreshToken.for_user(user=employee_1).access_token)
        ...
        cls.employee_2: UserDetail = employee_2
    ...
    def test_accessing_api_with_invalid_permission(self):
        """Method to test accessing project_member listing api with invalid permission."""
    ...
    def test_list_project_member(self):
        ...
        url: str = reverse("project_member:list")
        response: Response = self.client.get(url, HTTP_AUTHORIZATION=self.employee_1_token)
    ...
    def test_add_project_member_valid_input(self):
        ...
        response: Response = self.client.post(url, data=request_body, HTTP_AUTHORIZATION=self.employee_1_token)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

    def test_add_project_member_invalid_input(self):
        """Method to test adding proeject_member with incorrect input."""
        url: str = reverse("project_member:add")
        response: Response = self.client.post(url, HTTP_AUTHORIZATION=self.employee_1_token)
    ...
    def test_edit_project_member_valid_input(self):
        ...
        response: Response = self.client.put(url, data=request_body, HTTP_AUTHORIZATION=self.employee_1_token)
    ...
    def test_edit_project_member_invalid_input(self):
        ...
        url: str = reverse(self.update_url, kwargs={"project_member_id": self.project_member.project_member_id})
        response: Response = self.client.put(url, HTTP_AUTHORIZATION=self.employee_1_token)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_delete_project_member(self):
        ...
        response: Response = self.client.delete(url, HTTP_AUTHORIZATION=self.employee_1_token)
